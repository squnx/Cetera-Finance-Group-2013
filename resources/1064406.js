...
jQuery(function(){
   jQuery("div.svwp").prepend("<img src='../resources/2478872.gif' class='ldrgif' alt='loading...'/ >"); //change with YOUR loader image path   
});
...
jQuery.fn.slideViewerPro = function(settings) {
    ...
    .wrap(jQuery("<div style='width:"+ pictWidth +"px; overflow: hidden; position: relative; top: 0; left: 0'>"));
		...
		container.each(function(i) {
			//Modified typo for exchange with Alert: first function display alert, second display normal
			if(settings.typo)
			{
			jQuery(this).find("img").each(function(z) {
				jQuery(this).after("<span class='typo' style='position: absolute; font-size:11px; background-color:red; color:#fff; padding-left:180px; width:"+(pictWidth-12)+"px; margin: 0 0 0 -"+pictWidth+"px'><img src='images/NewWebsiteAlert.gif' style='vertical-align:bottom' />" + settings.alertText + "<\/span>");
			});
			}else{
			jQuery(this).find("img").each(function(z) {
			jQuery(this).after("<span class='typo' style='position: absolute; font-size:11px; padding-left:180px; width:"+(pictWidth-12)+"px; margin: 0 0 0 -"+pictWidth+"px'>"+jQuery(this).attr("alt")+"<\/span>");
			});
			}
			...
			jQuery(this).find("li").each(function(n) { 
				jQuery("div#thumbSlider" + j + " ul").append("<li><a title='" + jQuery(this).find("img").attr("alt") + "' href='#'><div class='numLabelAlert'>" + ++n + "</div><p class='tmbrdr'><\/p><\/a><\/li>");						
			});
			}else{
			jQuery(this).find("li").each(function(n) { 
				jQuery("div#thumbSlider" + j + " ul").append("<li><a title='" + jQuery(this).find("img").attr("alt") + "' href='#'><div class='numLabel'>" + ++n + "</div><p class='tmbrdr'><\/p><\/a><\/li>");						
			});	
			}
			...
			if(settings.typo)
			{								
					jQuery(this).find("p.tmbrdr").css({borderColor: settings.thumbsActiveBorderColor, opacity: settings.thumbsActiveBorderOpacity}).siblings().css('backgroundColor', '#ff6600');
					jQuery(this).parent().parent().find("p.tmbrdr").not(jQuery(this).find("p.tmbrdr")).css({borderColor: settings.thumbsBorderColor, opacity: settings.thumbsBorderOpacity}).siblings().css('backgroundColor', 'red');
			}else{
				
			jQuery(this).find("p.tmbrdr").css({borderColor: settings.thumbsActiveBorderColor, opacity: settings.thumbsActiveBorderOpacity}).siblings().css('backgroundColor', '#E5E5E5');
					jQuery(this).parent().parent().find("p.tmbrdr").not(jQuery(this).find("p.tmbrdr")).css({borderColor: settings.thumbsBorderColor, opacity: settings.thumbsBorderOpacity}).siblings().css('backgroundColor', '#fff');
			}
					var cnt = -(pictWidth*z);
			...
			jQuery("div#thumbSlider" + j).css({position: "relative", left: contBorder, top: settings.thumbsTopMargin+"px", width: settings.thumbs*thumbsWidth+((settings.thumbsRightMargin*settings.thumbs)-settings.thumbsRightMargin)+"px", height: thumbsHeight, textAlign: "center", overflow: "visible", margin: "0 0 0 2px"});
			...
			jQuery("div#thumbSlider" + j).find("p.tmbrdr").css({width: "16px", height: (thumbsHeight-tBorder*2) +"px", top: -(thumbsHeight) +"px", border: settings.thumbsBorderWidth +"px solid "+settings.thumbsBorderColor, opacity: settings.thumbsBorderOpacity});
			...
			jQuery("div#thumbSlider" + j + ", a#left" + j + ", a#right" + j).wrapAll("<div style='margin-top:-35px;width:"+ pictWidth +"px; display: "+uiDisplay+"' id='ui"+j+"'><\/div>");			
